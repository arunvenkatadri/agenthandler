...
`DurableTaskRunner` supplies milestone intent, persistent budgets, and recovery
coordination. A milestone verifier can return `report.as_result()` after running
this gate. See [durable tasks](durable-tasks.md) for storage and reconciliation
requirements; the application must still wire the cloud worker to that lifecycle.
